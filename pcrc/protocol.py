...
class Protocol(NamedTuple):
    ...
    def from_mc_version(cls, mc_version: str) -> "Protocol":
        return Protocol(
            mc_version=mc_version, protocol_number=KNOWN_MINECRAFT_VERSIONS[mc_version]
        )
...
ALL_PROTOCOL: List[Protocol] = list(
    filter(lambda o: isinstance(o, Protocol), globals().values())
)
SUPPORTED_MINECRAFT_VERSIONS: List[str] = list(
    map(lambda p: p.mc_version, ALL_PROTOCOL)
)
SUPPORTED_PROTOCOL_VERSIONS: List[int] = list(
    map(lambda p: p.protocol_number, ALL_PROTOCOL)
)
...
class MobTypeIds:
    ...
    def item(context: ConnectionContext) -> int:
        return (
            41
            if context.protocol_later_eq(MC_1_17_1.protocol_number)
            else 35
            if context.protocol_later_eq(MC_1_15_2.protocol_number)
            else 34
            if context.protocol_later_eq(MC_1_14_4.protocol_number)
            else 2
            if context.protocol_later_eq(MC_1_12_2.protocol_number)
            else -1
        )
    ...
    def phantom(context: ConnectionContext) -> int:
        return (
            ...
            if context.protocol_later_eq(MC_1_16_1.protocol_number)
            else 98
            if context.protocol_later_eq(MC_1_15_2.protocol_number)
            else 97
            if context.protocol_later_eq(MC_1_14_4.protocol_number)
