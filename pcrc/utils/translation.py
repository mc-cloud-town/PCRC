...
class Translation:
    def __init__(self):
        ...
        for lang in LANGUAGES:
            ...
            self.translations[lang] = self.__build_translation(
                YAML().load(resources_util.get_data(lang_file_path))
            )
        l1 = list(self.translations["en_us"].keys())
        l2 = list(self.translations["zh_cn"].keys())
        l3 = list(self.translations["zh_tw"].keys())
        ...
        def __build(obj: dict, path: str):
            for key, value in obj.items():
                child_path = (
                    key
                    if len(path) == 0
                    else (path if key == "." else path + "." + key)
                )
