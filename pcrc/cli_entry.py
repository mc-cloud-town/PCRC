...
def on_start_up():
    ...
    logger.info(
        "PCRC is open source, u can find it here: https://github.com/Fallen-Breath/PCRC"
    )
...
def show_help():
    logger.info(
        "Command list: help|start|stop|restart|exit|reload|auth|say|whitelist|set|status|list"
    )
...
def stop():
    ...
    if is_working():
        ...
        logger.info(
            'PCRC is not running, ignore. Enter "exit" if you want to exit PCRC'
        )
...
def set_option_commands(text: str, config: Config):
    ...
    try:
        ...
        logger.info(
            'Assign "{}" = "{}" ({}) now'.format(
                option, value, config.get_option_type(option).__name__
            )
        )
